...
|---|---|---|---|
| #70 Boat dynamics ([#194](https://github.com/reederphill/mobile-regatta/pull/194)) | The new physics moves #62's fixture boats, so `RegattaUITests/References/iPhone 17/prestart.png` must be re-recorded; CI refuses `-recordReferences` and the cloud container has no simulator | On a Mac: run the UI tests on `ticket-70-boat-dynamics` with `TEST_RUNNER_RECORD_REFERENCES=1`, check the PNG, commit it and push. The next session then runs the Haiku acceptance check and merges | 2026-09-25 |
