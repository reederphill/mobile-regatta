...
final class RaceAssemblyUITests: RaceUITestCase {
    ...
    /// the first rounding (a later leg). On the iOS 26.5 simulator the gun is about 20 s after the status
    /// line and the rounding about 20 s after the gun; each wait is 4 times that, for a slow CI runner, and
    /// with the launch they add up to 3.5 min, under the 5 min CI gives a test (`RaceUITestCase`). Leg 2
    /// is the short reach from the windward mark to the offset mark, a second or two of real time at 8×,
    /// so a poll can miss it: any leg past 1 shows the rounding.
    @MainActor func testPracticeRaceOnTheNewCoreReachesTheGunAndAFirstRounding() throws {
        ...
        XCTAssertTrue(status.waitForExistence(timeout: 20), "no race status line")
        ...
        XCTAssertTrue(waitForLabel(matching: ".*leg 1/.*", timeout: 80), "never started racing after the gun: \(status.label)")
        XCTAssertTrue(waitForLabel(matching: ".*leg ([2-9]|[1-9][0-9])/.*", timeout: 80),
