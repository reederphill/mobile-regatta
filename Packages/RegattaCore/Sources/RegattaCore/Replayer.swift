...
/// Re-simulates a race from its log (ADR 0002): builds `Race(setup, windSeed)` from the header,
/// feeds every input and seat event at its tick, and steps to the log's final tick. It never runs a
/// bot brain: bots' inputs are in the log like everyone else's.
public enum Replayer {
    ...
    public static func replay(_ log: RaceLog, requireMatchingVersion: Bool = true) throws -> Race {
    ...
    public static func digest(of log: RaceLog, requireMatchingVersion: Bool = true) throws -> UInt64 {
        try replay(log, requireMatchingVersion: requireMatchingVersion).digest()
    }
}
