...
@MainActor @Suite struct VisualCorrectionTests {
    ...
    @Test func halvesAnErrorInAboutFiftyMilliseconds() {
        var correction = VisualCorrection()
    ...
    @Test func snapsAnErrorLongerThanAHull() {
        ...
        correction.correct(shown: boat(at: Vec2(Boat.length + 0.01, 0), heading: 1), corrected: corrected)
        ...
        correction.correct(shown: boat(at: Vec2(Boat.length - 0.01, 0)), corrected: corrected)
        #expect(correction.positionError.x == Boat.length - 0.01, "one hull length or less eases")
    }
